...
(Обращение к двумерному массиву как к одномерному)
*/
...
void divideArrayToPositiveNegativeAndZiroNumbers(int* array, char rowCount, char collumnCount,
...
void fillTwoDimensionalArrayWhithRandomNumbers(int* array, char arrayRowCount, char arrayCollumnCount);
void printTwoDimensionalArray(int* array, char arrayRowCount, char arrayCollumnCount);
...
int main()
{
    ...
    fillTwoDimensionalArrayWhithRandomNumbers(&generatedArray, ROW_COUNT, COLLUMN_COUNT);

    printf("Generated array:\n");
    printTwoDimensionalArray(&generatedArray, ROW_COUNT, COLLUMN_COUNT);
    ...
    divideArrayToPositiveNegativeAndZiroNumbers(&generatedArray, ROW_COUNT, COLLUMN_COUNT,
    ...
    char* positiveNumberCount, char* negativeNumberCount, char* ziroCount)
{
    ...
    {
        for (char j = 0; j < collumnCount; j++)
        {
            int numberOfArray = array[i*rowCount + j];
...
void fillTwoDimensionalArrayWhithRandomNumbers(int* array, char arrayRowCount, char arrayCollumnCount)
{
    ...
    {
        for (char j = 0; j < arrayCollumnCount; j++)
        {
            array[i*arrayCollumnCount + j] = rand() % (2 * MAX_NUMBER_IN_ARRAY) - MAX_NUMBER_IN_ARRAY;
        }
    }
}
...
void printTwoDimensionalArray(int* array, char arrayRowCount, char arrayCollumnCount)
{
    for (char i = 0; i < arrayRowCount; i++)
    {
        for (char j = 0; j < arrayCollumnCount; j++)
        {
            printf("%5i", array[i*arrayRowCount + j]);
        }
        printf("\n");
    }
    printf("\n");
